...
- **ボスバーディスプレイ**: 報酬が利用可能になるまでの残り時間をリアルタイムでカスタマイズ可能なボスバーで表示します。目標達成で自動更新されます。
...
### ストレージオプション
- **YAML (デフォルト)**: プレイヤーデータをYAMLファイル（`playerdata.yml`）に保存します。
- **SQLite**: プレイヤーデータをSQLiteデータベース（`playerdata.db`）に保存します。大規模サーバー向けに推奨。
- **データ移行**: YAMLとSQLite間でプレイヤーデータを移行できます。
...
- **ストレージタイプ**: `config.yml`の`storage-type`で`yaml`または`sqlite`を選択できます。
...
- `/rewardmigrate <from> <to>`  
  プレイヤーデータをストレージ間で移行します（例: `/rewardmigrate yaml sqlite`）。YAMLからSQLite、またはその逆に移行できます。
...
プレイヤーデータ（累積時間、ストリーク、最後の報酬）を保存します。

- **YAML形式**: `plugins/LoginBonus/playerdata.yml` - デフォルトのストレージ形式
- **SQLite形式**: `plugins/LoginBonus/playerdata.db` - 大規模サーバー向けの高性能ストレージ
...
`config.yml`で`storage-type`を設定します:
...
1. サーバーを起動した状態で、`/rewardmigrate yaml sqlite`コマンドを実行します。
2. データ移行が完了したら、`config.yml`の`storage-type`を`sqlite`に変更します。
3. `/rewardreload`コマンドでリロードするか、サーバーを再起動します。

### SQLiteからYAMLへの移行

1. サーバーを起動した状態で、`/rewardmigrate sqlite yaml`コマンドを実行します。
   - **注意**: SQLiteからの移行はオンラインプレイヤーのみが対象です。
2. データ移行が完了したら、`config.yml`の`storage-type`を`yaml`に変更します。
3. `/rewardreload`コマンドでリロードするか、サーバーを再起動します。

### SQLiteの利点

- **パフォーマンス**: 大量のプレイヤーデータの読み書きが高速です。
- **データ整合性**: トランザクション処理により、データの整合性が保証されます。
- **スケーラビリティ**: 大規模サーバーに適しています。

### YAMLの利点

- **可読性**: テキストエディタで簡単に確認できます。
- **バックアップ**: ファイルのコピーだけで簡単にバックアップできます。
- **互換性**: 既存の多くのMinecraftプラグインで使用されている形式です。
...
5. （オプション）SQLiteストレージを使用する場合、`config.yml`の`storage-type`を`sqlite`に変更します。
6. 設定を変更した場合、`/rewardreload` コマンドを使用するか、再びサーバーを再起動します。

**注意**: SQLite JDBCドライバーはプラグインに含まれているため、追加のインストールは不要です。

...
- **報酬システム**: プレイヤーが設定された目標プレイ時間に達すると、自動で報酬を付与します。インベントリが満杯の場合は`/rewardclaim`で手動請求。
- **ボスバーディスプレイ**: 報酬が利用可能になるまでの残り時間をリアルタイムでカスタマイズ可能なボスバーで表示します。目標達成で自動削除。
- **自動報酬付与**: 目標時間に達すると、インベントリが許す限り自動で報酬を付与。満杯時はpending状態になります。
- **インベントリ満杯処理**: インベントリが満杯の場合、報酬は地面にドロップされます。
...
### プレイヤーコマンド
- `/rewardclaim`  
  プレイヤーが目標プレイ時間に達している場合、利用可能な報酬を請求します。インベントリが満杯の場合、アイテムはプレイヤーの場所にドロップされます。
- `/rewardstreak`  
  現在のストリーク日数を確認します。
...
- `/rewardforcestreak <player> <days>`  
  指定されたプレイヤーのストリークカウントを指定された日数に強制的に設定し、最後のストリーク日付をリセットします。
...
5. 設定を変更した場合、`/rewardreload` コマンドを使用するか、再びサーバーを再起動します。

## パーミッション

- 全てのプレイヤーコマンドは通常のプレイヤーが利用可能です。
- 管理者コマンドはOPステータスが必要です（パーミッションプラグインを使用する場合、適切なパーミッションが必要ですが、プラグインは現在OP権限をチェックしています）。

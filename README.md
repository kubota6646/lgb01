...
- **自動報酬付与**: 目標時間に達すると、インベントリが許す限り自動で報酬を付与。
...
4. 必要に応じて`config.yml`と`message.ymlを設定します。
5. 設定を変更した場合、`/rewardreload` コマンドを使用するか、再びサーバーを再起動します。

## パーミッション

- 全てのプレイヤーコマンドは通常のプレイヤーが利用可能です。
- 管理者コマンドはOPステータスが必要です（パーミッションプラグインを使用する場合、適切なパーミッションが必要ですが、プラグインは現在OPチェックを使用しています）。
